...
class Mime_Message
{
     ...
     *
     * @param Mime_Part $part
     */
    public function addPart(Mime_Part $part)
    {
        /**
    ...
    /**
     * Set Mime_Encode object for the message
     *
     * This can be used to set the boundary specifically or to use a subclass of
     * Mime_Encode for generating the boundary.
     *
     * @param Mime_Encode $mime
     */
    public function setMime(Mime_Encode $mime)
    {
        $this->_mime = $mime;
    }

    /**
     * Returns the Mime_Encode object in use by the message
     *
     ...
     *
     * Parts are seperated by the mime boundary as defined in Mime_Encode. If
     * {@link setMime()} has been called before this method, the Mime_Encode
     * object set by this call will be used. Otherwise, a new Mime_Encode object
     * is generated and used.
     *
     * @param string $EOL EOL string; defaults to {@link Mime_Const::LINEEND}
     * @return string
     */
    public function generateMessage($EOL = Mime_Const::LINEEND)
    {
        if (! $this->isMultiPart()) {
     ...
     * @return string
     */
    public function getPartHeaders($partnum, $EOL = Mime_Const::LINEEND)
    {
     ...
     * @return string
     */
    public function getPartContent($partnum, $EOL = Mime_Const::LINEEND)
    {
    ...
    protected static function _disassembleMime($body, $boundary)
    {
        ...
        if ($p===false) {
            throw new Mime_Exception('Not a valid Mime Message: End Missing');
        }
     ...
     * @return Mime_Message
     */
    public static function createFromMessage($message, $boundary, $EOL = Mime_Const::LINEEND)
    {
        $parts = Mime_Decode::splitMessageStruct($message, $boundary, $EOL);

        $res = new self();
        foreach ($parts as $part) {
            // now we build a new MimePart for the current Message Part:
            $newPart = new Mime_Part($part['body']);
